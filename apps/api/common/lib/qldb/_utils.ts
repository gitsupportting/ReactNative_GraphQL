import { createQldbWriter } from 'amazon-qldb-driver-nodejs';
import { Decimal, IonTypes, Reader, Timestamp, Writer } from 'ion-js';

export const prepareStatement = (query: string) => query.replace(/\n/g, '');

export const prepareWhere = (condition: object, table = 't') =>
    Object.keys(condition)
        .map(key => `${table}.${key} = ?`)
        .join(' AND ');

export const prepareSet = (values: object, table = 't') =>
    Object.keys(values)
        .map(key => `${table}.${key} = ?`)
        .join(', ');

export const prepareParameters = (args: any[]) => args.map(value => writeValueAsIon(value));

/**
 * Function which, given a reader and a path, traverses through the reader using the path to find
 * the value.
 * @param ionReader {Reader} The reader to operate on.
 * @param path {string[]} The path to find the value.
 * @returns {Array} The value obtained after traversing the path.
 */
export function getFieldValue(ionReader: Reader, path: string[]) {
    ionReader.next();
    ionReader.stepIn();
 ...
 * @param ionReader {Reader} The reader to operate on.
 * @param path {string[]} The path to find the value.
 * @returns {Array} The value, or undefined if the provided path does not exist.
 */
function recursivePathLookup(ionReader: Reader, path: string[]): any {
    if (path.length === 0) {
        ...
        if (ionReader.type() === IonTypes.LIST) {
            ...
            const structToReturn: Record<string, any> = {};
 ...
 * @returns {Array} The value obtained after traversing the path.
 */
export function getFieldValues(ionReader: Reader) {
    ionReader.next();

    return recursiveResolve(ionReader);
}
 ...
 * @returns {Array} The value, or undefined if the provided path does not exist.
 */
function recursiveResolve(ionReader: Reader): any {
    // If the path's length is 0, the current ionReader node is the value which should be returned.
    if (ionReader.type() === IonTypes.LIST) {
        const list = [];
        ionReader.stepIn(); // Step into the list.
        while (ionReader.next() != null) {
            const itemInList = recursiveResolve(ionReader);
            list.push(itemInList);
        }
        ...
        while (ionReader.depth() > currentDepth) {
            ...
            if (type === null) {
                ...
                structToReturn[ionReader.fieldName() || ''] = recursiveResolve(ionReader);
            }
        }
        return structToReturn;
    }
 ...
 * @param ionWriter {Writer} The Writer to pass the value into.
 * @throws Error: If the given value cannot be converted to Ion.
 */
export function writeValueAsIon(value: any, ionWriter = createQldbWriter()) {
    switch (typeof value) {
        case 'string':
